...
| --- | --- | --- |
| Entity | `name`, `icon`, `area_id`, `aliases`, `labels`, `categories`, `disabled_by`, `hidden_by` | `er.async_update_entity()` |
...
| Bulk set friendly name | Bulk-set the friendly-name override (`name`; `None` clears). Deliberately excludes `entity_id` — see Non-goals. | S | low |
...
- **Bulk `entity_id` renames / pattern-regex rewrites.** The API exists (`new_entity_id`), but nothing server-side rewrites references — the frontend's "rename in automations too" cannot be called by an integration — so a bulk id change silently breaks automations, scripts, templates, scenes, and dashboards for exactly the non-expert users least able to recover. Only the friendly-name override is offered; same exclusion for `new_unique_id` (provider-collision risk).
